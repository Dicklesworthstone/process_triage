...
use pt_common::{ProvenanceConfidence, ProvenanceConsentRequirement, PROVENANCE_PRIVACY_POLICY_VERSION};
...
fn provenance_debug_trace_fixture_uses_canonical_event_vocabulary() {
    ...
    for event in &events {
        ...
        assert!(event["privacy_version"].is_string(), "privacy_version required");
